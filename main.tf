...
resource "aws_ebs_volume" "this" {
  ...
  size              = "${element(var.external_volume_sizes, min(count.index / var.instance_count) % var.external_volume_count)}"

  encrypted  = true
  kms_key_id = "${element(coalescelist(list(var.external_volume_kms_key_arn), aws_kms_key.this.*.arn), 0)}"

  tags = "${merge(map("Name", format("%s-%s-%02d", var.name, var.external_volume_name_suffix, count.index + 1)), map("Terraform", "true"), var.tags, var.external_volume_tags)}"
}
